...
public static class Example21_ChatGptPlugins
{
    ...
    private static async Task RunKlarnaPluginAsync()
    {
        //--------------- Example of using Klarna ChatGPT plugin ------------------------

        var kernel = new KernelBuilder().WithLogger(ConsoleLogger.Log).Build();
        var userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/115.0";
        HttpClient httpClient = new();
        //httpClient.DefaultRequestHeaders.UserAgent
        //    .ParseAdd("Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/115.0");

        httpClient.DefaultRequestHeaders
            .Add("User-Agent", userAgent);

        var skill = await kernel.ImportChatGptPluginSkillFromUrlAsync(
            "Klarna",
        ...
        contextVariables.Set("countryCode", "US");
        var result = await kernel.RunAsync(contextVariables, skill["productsUsingGET"]);
